...
bool Ciudad::grabarDisco(Ciudad aux){

FILE *pCiudad = fopen("ciudad.dat","ab");
    if(pCiudad == NULL){return false;}

    bool guardo = fwrite(&aux, sizeof(Ciudad),1,pCiudad);
    fclose(pCiudad);
    return guardo;
...
void Ciudad::leerDisco(){
    FILE *pCiudad = fopen("ciudad.dat", "rb");
    Ciudad aux;
    if(pCiudad == NULL){
        cout << "Error de lectura" << endl;
        }

    while (fread(&aux, sizeof(Ciudad),1,pCiudad)==1)
        {
        aux.Mostrar();
        cout << endl;
        }
